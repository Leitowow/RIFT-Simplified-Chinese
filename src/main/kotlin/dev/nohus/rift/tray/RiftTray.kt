...
private fun getTrayMenuItems(
    ...
    return buildList {
        ...
        add(Separator)
        add(TrayMenuTextItem("警报", Res.drawable.window_loudspeaker_icon) { windowManager.onWindowOpen(WindowManager.RiftWindow.Alerts) })
        add(TrayMenuTextItem("星图", Res.drawable.window_map) { windowManager.onWindowOpen(WindowManager.RiftWindow.Map) })
        add(TrayMenuTextItem("预警总览", Res.drawable.window_satellite) { windowManager.onWindowOpen(WindowManager.RiftWindow.IntelFeed) })
        add(TrayMenuTextItem("预警提示", Res.drawable.window_bleedchannel) { windowManager.onWindowOpen(WindowManager.RiftWindow.IntelReports) })
        add(TrayMenuTextItem("角色", Res.drawable.window_characters) { windowManager.onWindowOpen(WindowManager.RiftWindow.Characters) })
        add(TrayMenuTextItem("资产", Res.drawable.window_assets) { windowManager.onWindowOpen(WindowManager.RiftWindow.Assets) })
        add(TrayMenuTextItem("行星工业", Res.drawable.window_planets) { windowManager.onWindowOpen(WindowManager.RiftWindow.PlanetaryIndustry) })
        add(TrayMenuTextItem("联系人", Res.drawable.window_contacts) { windowManager.onWindowOpen(WindowManager.RiftWindow.Contacts) })
        if (isJabberEnabled) {
            add(TrayMenuTextItem("通知", Res.drawable.window_sovereignty) { windowManager.onWindowOpen(WindowManager.RiftWindow.Pings) })
            add(TrayMenuTextItem("Jabber", Res.drawable.window_chatchannels) { windowManager.onWindowOpen(WindowManager.RiftWindow.Jabber) })
        }
        add(TrayMenuTextItem("设置", Res.drawable.window_settings) { windowManager.onWindowOpen(WindowManager.RiftWindow.Settings) })
        add(TrayMenuTextItem("关于", Res.drawable.window_evemailtag) { windowManager.onWindowOpen(WindowManager.RiftWindow.About) })
        add(Separator)
